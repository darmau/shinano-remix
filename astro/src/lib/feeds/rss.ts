...
export function formatPubDate(date: string | null | undefined): string {
...
function renderEnclosure(enclosure: FeedEnclosure | undefined): string {
  if (!enclosure) return "";
  return `<enclosure url="${enclosure.url}" type="${enclosure.type}" length="${enclosure.length}" />`;
}

function renderItem(entry: RssEntry): string {
...
export function buildRssChannel(params: {
  title: string;
  description: string;
  language: string;
  link: string;
  ...
  imageUrl?: string;
  entries: RssEntry[];
  followChallenge?: FollowChallenge;
}): string {
  const {
    title,
    description,
  ...
  const followBlock = followChallenge
    ? `<follow_challenge>
...
// === TipTap → HTML for full article RSS ===
...
const RSS_STYLE_MAP: Record<string, string> = {
  article:
    "font-family:'PingFang SC','Hiragino Sans','Microsoft YaHei',sans-serif;margin:0;padding:0;color:#0f172a;line-height:1.75;font-size:16px;background-color:#ffffff;",
  ...
  blockquote:
    "margin:1.75rem 0;padding:0.75rem 1rem;border-left:4px solid #c4b5fd;background-color:#f5f3ff;color:#4c1d95;font-style:italic;",
...
function getHeadingLevel(level?: number): number {
...
function getTextContent(children?: TipTapNode[]): string {
...
function applyRssMarks(text: string, marks?: TipTapMark[]): string {
      ...
      case "link": {
        ...
        const targetAttr =
          typeof mark.attrs?.target === "string"
            ? ` target="${escapeAttr(mark.attrs.target as string)}"`
            : "";
        const relAttr =
          typeof mark.attrs?.rel === "string"
            ? ` rel="${escapeAttr(mark.attrs.rel as string)}"`
            : "";
...
function renderRssChildren(children?: TipTapNode[]): string {
  if (!Array.isArray(children) || children.length === 0) return "";
  return children.map(renderRssTipTapNode).join("");
}

function renderRssTipTapNode(node?: TipTapNode): string {
...
export function renderTipTapToRssHtml(content: Json | null | undefined): string {
...
export function fallbackArticleContent(summary: string): string {
