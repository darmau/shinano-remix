...
1. **アクセス解析について**  
   本サイトは [Umami](https://umami.is) を利用し、サイトの改善を目的とした匿名のアクセス解析を行います。  
   収集される情報は以下の通りです。  
   - 訪問者の国または地域  
   - 使用デバイスの種類・OS情報  
   - ブラウザの言語・バージョン  
   ...
   これらの情報は**個人を特定しない形で集計**され、追跡や商業利用の目的で使用されることはありません。
...
4. ユーザーは法令および公序良俗に従い、以下の内容を含むコメントを投稿してはなりません。  
   - 法律や規制に違反する内容  
   - 誹謗中傷、差別、プライバシー侵害、知的財産権の侵害  
   - 広告、スパム、無関係なリンクや投稿  
...
1. 本サイトは、第三者サービス（Supabase、GitHub、Umami等）による障害・データ損失・通信中断等について責任を負いません。  
